...
threshold_peaks_one_indiv <- function(data_filt, save.plots = FALSE){
  ...
  fluo_under_first_domain <- trapz(inits[1]:ends[1],
                                 data_filt$values.C1[inits[1]:ends[1]])
  fluo_under_last_domain <- trapz(inits[length(inits)]:ends[length(inits)],
  ...
  for(domain in 2:(length(inits)-1)){
    if(ends[domain]>inits[domain+1]){ #should solve issues with overlapping peaks
      ...
      colocalised.C1 <- colocalised.C1+trapz(inits[domain]:ends[domain],
  ...
  if(ends[length(inits)-1]>inits[length(inits)]){ # should solve issues when the last peak overlaps, together with other loop above
    colocalised.C1 <- colocalised.C1-trapz(inits[length(inits)]:ends[length(inits)-1],
  ...
  intensity.C1.total <- trapz(1:pixel.size, data_filt$values.C1[1:pixel.size])
  ...
  data.frame(
    ...
    C1.total.true  =  trapz(inits[1]:ends[length(ends)],
